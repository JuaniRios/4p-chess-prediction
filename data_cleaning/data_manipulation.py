...
def delete_corners(board):
    '''Function to delete the unplayable corners of a 14x14 sized dictionary serving as a 4PChess baord.'''
    # the function is currently not in use as most algorithms will need the whole 14x14 board as a matrix input
    corners = []
    for i in range(1, 4):
        for c in "abc":
            corners += [f"{c}{i}"]
            ...
            for i in range(len(moves)): # go through each player's moves
                if moves[i] == ["O", "O"]: # if kingside castling
                    ...
                    if board[moves[i][1]][0] == 6 and moves[i][0] != moves[i][1]:
                        # if castling happens in the same round as some player has dropped out and there has not been a T, S or R,
